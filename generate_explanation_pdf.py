...
def generate_explanation_pdf(output_path: str = "EXPLANATION.pdf"):
    ...
    # Helper function to draw structured section cards using PyMuPDF HTMLBox
    def draw_section_card(top_y, height, title, content_paragraphs):
        ...
        page.insert_text(pymupdf.Point(46, top_y + 15), title, fontsize=10.0, fontname="hebo", color=(1, 1, 1))

        # Build HTML for card body
        html_parts = ['<div style="font-family: helvetica, sans-serif; font-size: 8.2pt; color: #263238; line-height: 1.35;">']
        for i, (header, body) in enumerate(content_paragraphs):
            margin_bottom = "5px" if i < len(content_paragraphs) - 1 else "0px"
            html_parts.append(
                f'<p style="margin: 0 0 {margin_bottom} 0;">'
                f'<b style="color: #1f3b8b;">&gt; {header}:</b> {body}'
                f'</p>'
            )
        html_parts.append('</div>')
        full_html = "".join(html_parts)

        # Render HTML box inside the card
        body_rect = pymupdf.Rect(46, top_y + 27, 549, top_y + height - 4)
        page.insert_htmlbox(body_rect, full_html)
    ...
    draw_section_card(104, 160, sec1_title, sec1_content)
    ...
    draw_section_card(272, 170, sec2_title, sec2_content)
    ...
    draw_section_card(450, 148, sec3_title, sec3_content)
    ...
    draw_section_card(606, 174, sec4_title, sec4_content)
